...
class SimplifiedBattleEngine:
    ...
    def select_move(
        ...
        if not selected_move:
            ...
            # 유효한 기술이 하나도 없으면 어쩔 수 없이 전체 목록 사용
            if not valid_moves:
    ...
    def _create_default_move(self, pokemon: SimplifiedPokemon) -> Optional[SimplifiedMove]:
        ...
        # 내부 클래스 정의
        class DefaultMove:
            def __init__(self, pokemon : SimplifiedPokemon):
                self.id = 'tackle'
                self.base_power = 40
                self.accuracy = 100
                self.priority = 0
    ...
    def _calculate_damage(
        ...
        if verbose:
             ...
             self.logger.info(f" - Multiplier Applied: {final_damage / base_damage:.2f}x")

        return max(1, int(final_damage))
    
    def _end_of_turn(self, battle: SimplifiedBattle):
        """턴 종료 처리"""
        # 능력치 타이머 업데이트 (만료된 boost 해제)
